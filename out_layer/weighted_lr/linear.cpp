...
class l2r_wlr_fun: public function
{
public:
	l2r_wlr_fun(const problem *prob, double *lambda);
	~l2r_wlr_fun();
...
l2r_wlr_fun::l2r_wlr_fun(const problem *prob, double *lambda)
{
	int l=prob->l;
...
double l2r_wlr_fun::fun(double *w)
{
	int i;
	double f=0;
	double *y=prob->y;
...
void l2r_wlr_fun::grad(double *w, double *g)
{
	int i;
	double *y=prob->y;
	int l=prob->l;
	...
	for(i=0;i<w_size;i++)
		g[i] = w[i]*lambda[i] + g[i];
}

int l2r_wlr_fun::get_nr_variable(void)
{
	return prob->n;
}

void l2r_wlr_fun::Hv(double *s, double *Hs)
{
	int i;
	int l=prob->l;
	int w_size=get_nr_variable();
	...
	for(i=0;i<w_size;i++)
		Hs[i] = s[i]*lambda[i] + Hs[i];
	delete[] wa;
}

void l2r_wlr_fun::Xv(double *v, double *Xv)
{
	int i;
	int l=prob->l;
	feature_node **x=prob->x;
...
void l2r_wlr_fun::XTv(double *v, double *XTv)
{
	int i;
	int l=prob->l;
	int w_size=get_nr_variable();
...
{
public:
	l2r_wlr_fun2(const problem *prob);
	~l2r_wlr_fun2();

	void get_alpha(double *w, double *alpha);
...
l2r_wlr_fun2::l2r_wlr_fun2(const problem *prob)
{
	int l=prob->l;
...
void l2r_wlr_fun2::get_alpha(double *w, double *alpha)
{
	int i;
	double *y=prob->y;
	int l=prob->l;
...
int l2r_wlr_fun2::get_nr_variable(void)
{
	return prob->n;
}

void l2r_wlr_fun2::Xv(double *v, double *Xv)
{
	int i;
	int l=prob->l;
	feature_node **x=prob->x;
...
void l2r_wlr_fun2::XTv(double *v, double *XTv)
{
	int i;
	int l=prob->l;
	int w_size=get_nr_variable();
	...
	double Cp, double Cn)
{
	...
	while(newton_iter < max_newton_iter)
	{
		...
		for(s=0; s<active_size; s++)
		{
			...
			//double Gp = Grad[j]+1;
			double Gp = Grad[j]+lambda[j];
			//double Gn = Grad[j]-1;
			double Gn = Grad[j]-lambda[j];
			double violation = 0;
			if(w[j] == 0)
			{
				if(Gp < 0)
		...
		for(j=0; j<w_size; j++)
		{
			...
			if(wpd[j] != 0)
				w_norm_new += lambda[j]*fabs(wpd[j]);
		}
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	double primal_solver_tol = eps/prob->l;
	...
	{ 
		case L2R_WLR:
		{
			double *lambda = new double[prob->n];
			for(int i = 0; i < prob->n; i++)
				lambda[i] = param->lambda[i];
			fun_obj=new l2r_wlr_fun(prob, lambda);
			double tol = 1e-7;
			TRON tron_obj(fun_obj, tol, eps_cg);
		...
		case L1R_LR:
		{
			...
			prob_col.lambda = new double[prob->n];
			for(int i = 0; i < prob->n; i++)
				prob_col.lambda[i] = param->lambda[i];
...
model* train(const problem *prob, const parameter *param)
{
	...
	model_->nr_train_samples = prob->l ;
	model_->alpha = Malloc(double, prob->l);
	...
	else
	{
		...
		{
			if(nr_class == 2 || param->solver_type == L2R_WLR)
			{
				model_->w=Malloc(double, w_size);

				int e0 = start[0]+count[0];
				if( param->solver_type == L2R_WLR || param->solver_type == L1R_LR )
				{
					for(k=0; k<sub_prob.l; k++)
						sub_prob.y[k] = prob->y[perm[k]];
				}
				else
				{
					k=0;
				...
				l2r_wlr_fun2 *l2r=new l2r_wlr_fun2(prob);
				l2r->get_alpha(model_->w, model_->alpha);
			}
			else
			{
