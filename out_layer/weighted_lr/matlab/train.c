...
{
	mexPrintf(
	"Usage: model = train(training_label_vector, training_instance_matrix, lambda_vector, 'liblinear_options', 'col');\n"
	"liblinear_options:\n"
	...
	"	 8 -- L2-regularized weighted logistic regression (primal)\n"
	"  for regression\n"
...
int read_lambda(const mxArray *lambda_vec, parameter *param, const problem *prob)
{
	param->lambda = NULL ;
	int lambda_num = mxGetM(lambda_vec);
	if( lambda_num<prob->n )
	{
		mexPrintf("Length of lambda vector must be no less than the feature dimensions\n");
		return -1 ;
	}
	param->lambda = Malloc( double, prob->n ) ;
	double *lambda = mxGetPr(lambda_vec) ;
	for( int i=0 ; i<prob->n ; i++ )
		param->lambda[i] = lambda[i] ;
	return 0 ;
}
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 1 && nrhs < 6)
	{
		...
		{
			mexPrintf("Error: label vector, instance matrix and lambda vector must be double\n");
			fake_answer(nlhs, plhs);
			return;
		}
		...
		//lambda
		if( read_lambda( prhs[2], &param, &prob ) )
		{
			destroy_param(&param);
			free(prob.y);
			free(prob.x);
